...
with engine.connect() as connection:
    ...
    """))
    # connection.execute(text("""ALTER TABLE movies
    #                         ADD poster TEXT"""))
    connection.commit()
...
def add_movie(title, year, rating, poster):
    ...
    with engine.connect() as connection:
        try:
            ...
            connection.execute(text("INSERT INTO movies (title, year, rating, poster) "
...
def delete_movie(title):
    ...
    with engine.connect() as connection:
        try:
            params = {'title': title}
            connection.execute(text(
                "DELETE FROM movies WHERE title = :title"), params)
            connection.commit()
        except Exception as e:
            print(f"Error: {e}")
